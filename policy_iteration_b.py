...
class State:
    def __init__(self, cars, req_e, ret_e, rental, mv_cost, max_cars, max_mv_cars, free_mv_cars, free_lots, ex_cost):
        self.cars = cars
        self.updates = {}
        ...
        for m in range(-max_mv_cars, max_mv_cars + 1):
            if cars[0] - m >= 0 and cars[0] - m <= max_cars and cars[1] + m >= 0 and cars[1] + m <= max_cars:
                reward = -mv_cost * (max(m - free_mv_cars, 0) if m > 0 else -m)
                if cars[0] - m > free_lots:
                    reward -= ex_cost
                if cars[1] + m > free_lots:
                    reward -= ex_cost
                self.actions[m] = ((cars[0] - m) * (max_cars + 1) + cars[1] + m, reward)


class CarRental:
    def __init__(self, req_e, ret_e, rental, mv_cost, max_cars, max_mv_cars, free_mv_cars, free_lots, ex_cost):
        self.values = np.zeros((max_cars + 1, max_cars + 1))
        self.policies = np.zeros((max_cars + 1, max_cars + 1), dtype=int)
        with Pool(cpu_count()) as p:
            self.__states = p.map(StateFactory(State, req_e, ret_e, rental, mv_cost, max_cars, max_mv_cars, free_mv_cars, free_lots, ex_cost), [(i, j) for i in range(max_cars + 1) for j in range(max_cars + 1)])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Policy Iteration (Exercise 4.7).")
    ...
    parser.add_argument("--free_mv_cars", help="maximum number of cars that can be moved from the first location to the second location for free (default: 1)", type=int, default=1)
    parser.add_argument("--free_lots", help="number of spaces that can park for free at each location (default: 10)", type=int, default=10)
    parser.add_argument("--ex_cost", help="cost of a second parking lot (default: 4)", type=int, default=4)
    ...
    print("Initializing...")
    g = CarRental((args.req_e1, args.req_e2), (args.ret_e1, args.ret_e2), args.rental, args.mv_cost, args.max_cars, args.max_mv_cars, args.free_mv_cars, args.free_lots, args.ex_cost)
    print("Training...")
